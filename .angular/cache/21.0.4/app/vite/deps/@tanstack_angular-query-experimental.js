...
function dehydrateMutation(mutation) {
  ...
  }, mutation.options.scope && { scope: mutation.options.scope }), mutation.meta && { meta: mutation.meta });
}
...
function hydrate(client, dehydratedState, options) {
  ...
  mutations.forEach((_a) => {
    var _b = _a, { state } = _b, mutationOptions2 = __objRest(_b, ["state"]);
    mutationCache.build(
      client,
      __spreadValues(__spreadValues(__spreadValues({}, client.getDefaultOptions().hydrate?.mutations), options?.defaultOptions?.mutations), mutationOptions2),
      state
    );
  });
  queries.forEach(
    ({ queryKey, state, queryHash, meta, promise, dehydratedAt }) => {
      ...
      if (query) {
        ...
        if (state.dataUpdatedAt > query.state.dataUpdatedAt || hasNewerSyncData) {
          const _a = state, { fetchStatus: _ignored } = _a, serializedState = __objRest(_a, ["fetchStatus"]);
    ...
    this.#retryer = createRetryer({
      ...
      onCancel: (error) => {
        if (error instanceof CancelledError && error.revert) {
          this.setState(__spreadProps(__spreadValues({}, this.#revertState), {
            fetchStatus: "idle"
          }));
        }
  ...
  #dispatch(action) {
    const reducer = (state) => {
      switch (action.type) {
        ...
        case "success":
          const newState = __spreadValues(__spreadProps(__spreadValues(__spreadValues({}, state), successState(action.data, action.dataUpdatedAt)), {
...
var QueryClient = class {
  ...
  isFetching(filters) {
    return this.#queryCache.findAll(__spreadProps(__spreadValues({}, filters), { fetchStatus: "fetching" })).length;
  }
  isMutating(filters) {
    return this.#mutationCache.findAll(__spreadProps(__spreadValues({}, filters), { status: "pending" })).length;
  }
  /**
  ...
  setQueryData(queryKey, updater, options) {
    ...
    return this.#queryCache.build(this, defaultedOptions).setData(data, __spreadProps(__spreadValues({}, options), { manual: true }));
  }
...
// node_modules/@tanstack/angular-query-experimental/providers.mjs
